...
class HomepageParser:
    card_marker: str | None = None
    max_card_length = 5000

    def card_block_for_anchor(self, page_html: str, match: re.Match[str]) -> str:
        if not self.card_marker:
            return match.group("body")

        start = page_html.rfind(self.card_marker, 0, match.start())
        if start == -1:
            return match.group("body")

        next_start = page_html.find(self.card_marker, match.end())
        if next_start == -1:
            next_start = min(len(page_html), start + self.max_card_length)
    ...
    def extract_campaigns(self, source: Any, page_html: str, limit: int) -> list[dict[str, Any]]:
        ...
        for match in anchor_pattern.finditer(page_html):
            ...
            card_block = self.card_block_for_anchor(page_html, match)
            ...
            title, reward_summary = self.title_and_reward(card_block, text)
            ...
            external_id = self.external_id(normalized_url)
